...
import { getUserLink } from "../graphql/queries";
import { updateUserLink } from "../graphql/mutations";
const client = generateClient();
export default function UserLinkUpdateForm(props) {
  const {
    id: idProp,
    userLink: userLinkModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userLinkRecord
      ? { ...initialValues, ...userLinkRecord }
      : initialValues;
    setProfessionalId(cleanValues.professionalId);
    setProfessionalEmail(cleanValues.professionalEmail);
    setProfessionalFirstName(cleanValues.professionalFirstName);
    setProfessionalLastName(cleanValues.professionalLastName);
    setClientId(cleanValues.clientId);
    setClientEmail(cleanValues.clientEmail);
    setClientFirstName(cleanValues.clientFirstName);
    setClientLastName(cleanValues.clientLastName);
    setClientViewable(cleanValues.clientViewable);
    setSubscriptionId(cleanValues.subscriptionId);
    setErrors({});
  };
  const [userLinkRecord, setUserLinkRecord] = React.useState(userLinkModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserLink.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserLink
        : userLinkModelProp;
      setUserLinkRecord(record);
    };
    queryData();
  }, [idProp, userLinkModelProp]);
  React.useEffect(resetStateValues, [userLinkRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateUserLink.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userLinkRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserLinkUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userLinkModelProp)}
