...
import { createNotifications, createProfessionalReviews } from "../graphql/mutations";
import { listAssessmentAnswers, listAssessmentScores, listCreatePlans, listGeneralInformations, listProfessionalReviews, listUserLinks } from "../graphql/queries";
...
export default function ViewReports() {
  ...
  useEffect(() => {
    ...
    const fetchPlan = async () => {
      ...
      try {
        console.log("Fetching CreatePlan for userId:", connectedClient.clientId);

        const response = await client.graphql({
          query: listCreatePlans,
          variables: { filter: { userId: { eq: connectedClient.clientId } } },
        });
  ...
  function openConfirmReview(id, type) {
    setPendingReview({ id, type });
    setConfirmReview(true);
  }
  async function handleConfirmReview() {
    if (!pendingReview.id || !pendingReview.type) {
    ...
    const professionalName = `${firstName} ${lastName}`.trim() || "Unknown";

    // Get professional role from GeneralInformation
    ...
    let professionalRole = "Unknown Role";
    const items = scoreResponse.data.listGeneralInformations?.items;
    if (items && items.length > 0) {
      professionalRole = items[0].ProfessionalRole || professionalRole;
    }

    let reviewedId = pendingReview.id;

    // If reviewing a health plan, get the CreatePlan id for this user instead
    if (pendingReview.type === "health plan") {
      try {
        // Update original notification status
        const userLinkResponse = await client.graphql({
          query: listUserLinks,
          variables: {
            filter: { clientId: { eq: connectedClient.clientId } }
          }
        });

        const userLinks = userLinkResponse.data.listUserLinks?.items || [];
        const clientEmail = userLinks[0].clientEmail;
        ...
        const newNote = {
          ...
          RecommedationNote: `${responderFirstName} ${responderLastName} has finalized their review for your health plan`,
      ...
      const planResponse = await client.graphql({
        query: listCreatePlans,
        variables: {
          filter: { userId: { eq: connectedClient.clientId } },
        },
      });
      const plans = planResponse.data.listCreatePlans?.items || [];
      if (plans.length > 0) {
        reviewedId = plans[0].id;  // or logic to pick the right plan if multiple
      } else {
        alert("No health plan found for this user.");
        setConfirmReview(false);
        return;
      }
    } else {
      try {
        ...
        const newNote = {
          ...
          RecommedationNote: `${responderFirstName} ${responderLastName} has finalized their review for your assessment`,
    ...
    try {
      await client.graphql({
        query: createProfessionalReviews,
        variables: { input },
      });
      ...
      setConfirmReview(false);
      setPendingReview({ id: null, type: null });
    }

  }


  const [pendingReview, setPendingReview] = useState({ id: null, type: null });
  ...
  // --- Client name + current date ---
  const clientNameLine = `Client Name: ${connectedClient.firstName} ${connectedClient.lastName}`;
  const today = new Date();
 const currentDateLine = `Date: ${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, "0")}-${String(today.getDate()).padStart(2, "0")}`;
  [clientNameLine, currentDateLine].forEach((line) => {
    doc.text(line, 20, y);
    y += 8;
  });
...
addSelectedFields(selectedFields);
  ...
  const handleDownloadPdf = async () => {
    ...
    const rawDate = selectedAssessment?.createdAt;

    const assessmentDate = rawDate
      ? new Date(rawDate).toISOString().split("T")[0] // gives "YYYY-MM-DD"
      : "N/A";


    // then render them
    [
      `Client Name: ${connectedClient.firstName} ${connectedClient.lastName}`,
      ...
      {viewFilter === "assessments" ? (
        assessments.length > 0 ? (
          <>
            ...
            <button
              onClick={() => openConfirmReview(selectedId, "assessment")}
              disabled={!selectedId}
              style={{ padding: "10px 20px", marginBottom: 20, borderRadius: 8, cursor: selectedId ? "pointer" : "not-allowed", width: "100%", }}
            >
        ...
        >
          ...
          {filteredPlans.length > 0 ? (
            <>
              ...
              <button
                onClick={() => {
                  if (filteredPlans.length > 0) openConfirmReview(filteredPlans[0].id, "health plan");
                  else alert("No health plan found.");
                }}
                style={{ padding: "10px 20px", borderRadius: 8, width: "100%" }}
              >
      ...
      {confirmReview && (
        <div style={{
          ...
          <div style={{
            ...
            <p>
              Are you sure you want to finalize the review for this{" "}
              {pendingReview.type === "assessment" ? "assessment" : "health plan"}?
            </p>
            <div style={{ marginTop: 24, display: "flex", justifyContent: "center", gap: 16 }}>
              <button
                onClick={handleConfirmReview}
                style={{ padding: "10px 20px", borderRadius: 8, backgroundColor: "#004a99", color: "white", cursor: "pointer" }}
              >
                Yes
              </button>
              <button
