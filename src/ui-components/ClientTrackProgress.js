...
const { connectedClient } = useConnectedClient();
  ...
  // Discontinue modal
  const [viewingCompletedType, setViewingCompletedType] = useState("successful"); // or "discontinued"
...
const [isMobile, setIsMobile] = useState(window.innerWidth <= 768);
  ...
  const fetchPlans = async () => {
    const activeItems = await fetchPlansByStatus(connectedClient.clientId, false, false);
  ...
  return (
    <>
      ...
      <div style={{...styles.container, width: isMobile? "100" : "100"}}>
        ...
        {plans.map((plan) => {
          ...
          return (
            <div key={plan.id} style={{...styles.planCard, width: isMobile? 380 : 500}}>
...
<div style={{ marginTop: 40, textAlign: "center" }}>
  <h2 style={{ color: "#181a1bff" }}>Past Actions</h2>
  <div style={{ marginBottom: 16, display: "flex", justifyContent: "center", gap: 12
    ...
    <button
      onClick={() => setViewingCompletedType("successful")}
      style={{
        padding: "10px 24px",
        borderRadius: 12,
        border:
          viewingCompletedType === "successful"
            ? "2px solid #0077cc"
            : "2px solid #ccc",
        backgroundColor:
          viewingCompletedType === "successful" ? "#d9eaff" : "#fff",
        color: viewingCompletedType === "successful" ? "#004080" : "#666",
        fontWeight: "600",
        cursor: "pointer",
        minWidth: 140,
    ...
    <button
      onClick={() => setViewingCompletedType("discontinued")}
      style={{
        padding: "10px 24px",
        borderRadius: 12,
        border:
          viewingCompletedType === "discontinued"
            ? "2px solid #0077cc"
            : "2px solid #ccc",
        backgroundColor:
          viewingCompletedType === "discontinued" ? "#d9eaff" : "#fff",
        color: viewingCompletedType === "discontinued" ? "#004080" : "#666",
        fontWeight: "600",
        cursor: "pointer",
        minWidth: 140,
        transition: "all 0.3s ease",
...
{viewingCompletedType === "successful" && successfulPlans.length > 0 && (
  <>
    {successfulPlans.map((plan) => (
      <div key={plan.id} style={{ ...styles.planCard, borderColor: "#e6f9eaff", width: isMobile? 380 : 500 }}>
        <h2 style={{ ...styles.planTitle, color: "#28a745" }}>{plan.Action}</h2>
        <p style={styles.planText}><strong>Question:</strong> {plan.ActionQuestion || "Unknown"}</p>
        <p style={styles.planText}><strong>Completed:</strong> {plan.SuccessfulDate || "Unknown"}</p>
        <p style={styles.planText}><strong>Frequency:</strong> {plan.Frequency || "Unknown"}</p>
        <p style={styles.planText}><strong>Note:</strong> {plan.ActionNote || "None"}</p>
        <div style={{ marginTop: 20, display: "flex", justifyContent: "center" }}>
          <PercentageWheel percentage={plan.Percentage ?? 0} color="#28a745" />
        </div>
      </div>
    ))}
  </>
...
{viewingCompletedType === "discontinued" && discontinuedPlans.length > 0 && (
  <>
    {discontinuedPlans.map((plan) => (
      <div key={plan.id} style={{ ...styles.planCard, borderColor: "#e6f9eaff", opacity: 0.85, width: isMobile? 380 : 500 }}>
        <h2 style={{ ...styles.planTitle, color: "#cc4444" }}>{plan.Action}</h2>
        <p style={styles.planText}><strong>Question:</strong> {plan.ActionQuestion || "Unknown"}</p>
        <p style={styles.planText}><strong>Discontinued:</strong> {plan.DiscontinuedDate || "Unknown"}</p>
        <p style={styles.planText}><strong>Frequency:</strong> {plan.Frequency || "Unknown"}</p>
        <p style={styles.planText}><strong>Note:</strong> {plan.ActionNote || "None"}</p>
        <div style={{ marginTop: 20, display: "flex", justifyContent: "center" }}>
          <PercentageWheel percentage={plan.Percentage ?? 0} color="#cc4444" />
        </div>
      </div>
    ))}
  </>
