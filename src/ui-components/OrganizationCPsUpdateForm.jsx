...
import { getOrganizationCPs } from "../graphql/queries";
import { updateOrganizationCPs } from "../graphql/mutations";
const client = generateClient();
export default function OrganizationCPsUpdateForm(props) {
  const {
    id: idProp,
    organizationCPs: organizationCPsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = organizationCPsRecord
      ? { ...initialValues, ...organizationCPsRecord }
      : initialValues;
    setOrganizationId(cleanValues.OrganizationId);
    setOrganizationCPFirstName(cleanValues.OrganizationCPFirstName);
    setOrganizationCPLastName(cleanValues.OrganizationCPLastName);
    setOrganizationCPEmail(cleanValues.OrganizationCPEmail);
    setOrganizationCPRole(cleanValues.OrganizationCPRole);
    setPrimaryDepartment(cleanValues.primaryDepartment);
    setPrimaryProgramsUnit(cleanValues.primaryProgramsUnit);
    setPrimaryService(cleanValues.primaryService);
    setOrganizationAdministrator(cleanValues.OrganizationAdministrator);
    setPrimaryOrganizationAdministrator(
      cleanValues.PrimaryOrganizationAdministrator
    );
    setErrors({});
  };
  const [organizationCPsRecord, setOrganizationCPsRecord] = React.useState(
    organizationCPsModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getOrganizationCPs.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getOrganizationCPs
        : organizationCPsModelProp;
      setOrganizationCPsRecord(record);
    };
    queryData();
  }, [idProp, organizationCPsModelProp]);
  React.useEffect(resetStateValues, [organizationCPsRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          OrganizationId,
          OrganizationCPFirstName: OrganizationCPFirstName ?? null,
          OrganizationCPLastName: OrganizationCPLastName ?? null,
          OrganizationCPEmail,
          OrganizationCPRole: OrganizationCPRole ?? null,
          primaryDepartment: primaryDepartment ?? null,
          primaryProgramsUnit: primaryProgramsUnit ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateOrganizationCPs.replaceAll("__typename", ""),
            variables: {
              input: {
                id: organizationCPsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "OrganizationCPsUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || organizationCPsModelProp)}
