...
export default function DashboardOrganization() {
    ...
    const [filterOrganization, setFilterOrganization] = useState(""); // for dropdown filter
    ...
    const fetchOrganizations = async () => {
        try {
            const { data } = await client.graphql({ query: listOrganizationInformations });
            const orgs = data?.listOrganizationInformations?.items || [];
            setOrganizations(orgs);
            setTotalOrganizations(orgs.length); // total organizations stat
        } catch (err) {
    ...
    const fetchOrganizationInfo = async () => {
        try {
            ...
            setOrganizations(orgs); // new state
            setTotalOrganizations(orgs.length); // new stat

            // If you still need to set Departments/Programs/Services for the first org
            const firstOrg = orgs[0];
            if (firstOrg) {
                setDepartments(firstOrg.Departments || []);
                setPrograms(firstOrg.ProgramsUnits || []);
                setServices(firstOrg.Services || []);
            }
    ...
    const fetchProfessionals = async () => {
        try {
            ...
            const pros = all.map(cp => ({
                id: cp.id,
                email: cp.OrganizationCPEmail,
                firstName: cp.OrganizationCPFirstName,
                lastName: cp.OrganizationCPLastName,
                userId: cp.userId,
                primaryDepartment: cp.primaryDepartment || "",
                primaryProgramsUnit: cp.primaryProgramsUnit || "",
                primaryService: cp.primaryService || "",
                organizationId: cp.organizationId || "", // <-- add this
            }));
...
const fetchAllClients = async (professionalsList) => {
  try {
    // 1️⃣ Fetch all linked clients
    const { data: linksData, errors: linksErrors } = await client.graphql({ query: listUserLinks });
    if (linksErrors) console.error("UserLinks GraphQL errors:", linksErrors);
    const links = linksData?.listUserLinks?.items || [];
    const professionalEmails = professionalsList.map(p => p.email);

    const linkedClients = links
  ...
  .map(link => {
    ...
    return {
      ...
      primaryService: pro?.primaryService || "",
       organizationId: pro?.organizationId || "", // optional fallback
      professionalOrganizationId: pro?.organizationId || "", // THIS is critical
    };
  });


    // 2️⃣ Fetch all individuals (standalone + linked, we filter linked manually)
    const { data: generalData, errors: generalErrors } = await client.graphql({
      query: listGeneralInformations,
      variables: { filter: { ProfileType: { eq: "Professional" } } },
    });
    if (generalErrors) console.error("GeneralInformations GraphQL errors:", generalErrors);

    const allGeneralUsers = generalData?.listGeneralInformations?.items || [];

    // 3️⃣ Keep only standalone users (not linked)
    const linkedClientIds = new Set(linkedClients.map(c => c.clientId));
    const standaloneClients = allGeneralUsers
      .filter(u => !linkedClientIds.has(u.userId))
      .map(u => ({
        id: u.id,
        clientId: u.userId,
        email: u.Email,
        ...
        professionalLastName: null,
        createdAt: u.createdAt || new Date().toISOString(),
        primaryDepartment: u.Department || "",
        primaryProgramsUnit: u.ProgramsUnit || "",
        primaryService: u.Service || "",
        organizationId: u.userId, // fallback for individual
        businessName: u.BusinessName || "",
      }));

    // 4️⃣ Merge linked + standalone
    const allClients = [...linkedClients, ...standaloneClients];
...
useEffect(() => {
  ...
  const filteredList = list.filter(item => {
    ...
    const matchesProFilter =
      view === "clients"
        ? !clientFilterPro || (item.professionalEmail || "").toLowerCase() === clientFilterPro.toLowerCase()
        : true;

    // --- ORGANIZATION / BUSINESS FILTER ---
    const matchesOrganization =
  !filterOrganization || // no filter
  (item.professionalOrganizationId && String(item.professionalOrganizationId) === String(filterOrganization)) || // linked client
  (item.organizationId && String(item.organizationId) === String(filterOrganization)) || // standalone client
  (item.businessName && item.businessName.toLowerCase() === String(filterOrganization).toLowerCase()); // business name match
    ...
    const matchesDateFilter =
      !createdAt || (!start || createdAt >= start) && (!end || createdAt <= end);

    // --- DEPARTMENT / PROGRAM / SERVICE FILTERS ---
    const matchesDepartment = !filterDepartment || item.primaryDepartment === filterDepartment;
    const matchesProgram = !filterProgram || item.primaryProgramsUnit === filterProgram;
    const matchesService = !filterService || item.primaryService === filterService;
    ...
    // --- DEBUGGING LOG ---
    if (!result && filterOrganization) {
      console.log("Filtered out:", item.firstName, item.lastName, "Org:", item.organizationId, "Business:", item.businessName);
    }
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
            ...
            <div style={containerStyle2}>
                ...
                <div style={statsRow}>
                    <div style={statLine}>
                        Total Organizations
                        <div style={highlightNumber}>{totalOrganizations}</div>
                    </div>
                    <div style={statLine}>
                        Plans Paid For
...
<div style={containerStyle2}>
  ...
  <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
    ...
    <select
      ...
      style={{ ...inputStyle, width: 200 }} // same style as others
    >
