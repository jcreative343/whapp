...
import { getGeneralInformation } from "../graphql/queries";
import { updateGeneralInformation } from "../graphql/mutations";
const client = generateClient();
export default function GeneralInformationUpdateForm(props) {
  const {
    id: idProp,
    generalInformation: generalInformationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = generalInformationRecord
      ? { ...initialValues, ...generalInformationRecord }
      : initialValues;
    ...
    setBusinessEIN(cleanValues.BusinessEIN);
    setBusinessCountry(cleanValues.BusinessCountry);
    setBusinessAddress(cleanValues.BusinessAddress);
    setBusinessCity(cleanValues.BusinessCity);
    setBusinessState(cleanValues.BusinessState);
    setBusinessZipCode(cleanValues.BusinessZipCode);
    setBusinessPhoneNumber(cleanValues.BusinessPhoneNumber);
    setBusinessWebsite(cleanValues.BusinessWebsite);
    setOrganizationAdministrator(cleanValues.OrganizationAdministrator);
    setOrganizationEmployee(cleanValues.OrganizationEmployee);
    setDepartment(cleanValues.Department);
    setService(cleanValues.Service);
    setProgramsUnit(cleanValues.ProgramsUnit);
    setProfileType(cleanValues.ProfileType);
    setErrors({});
  };
  const [generalInformationRecord, setGeneralInformationRecord] =
    React.useState(generalInformationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getGeneralInformation.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getGeneralInformation
        : generalInformationModelProp;
      setGeneralInformationRecord(record);
    };
    queryData();
  }, [idProp, generalInformationModelProp]);
  React.useEffect(resetStateValues, [generalInformationRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          OrganizationAdministrator: OrganizationAdministrator ?? null,
          OrganizationEmployee: OrganizationEmployee ?? null,
          Department: Department ?? null,
          Service: Service ?? null,
          ProgramsUnit: ProgramsUnit ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateGeneralInformation.replaceAll("__typename", ""),
            variables: {
              input: {
                id: generalInformationRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "GeneralInformationUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || generalInformationModelProp)}
