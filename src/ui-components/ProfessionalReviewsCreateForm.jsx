...
import { createProfessionalReviews } from "../graphql/mutations";
const client = generateClient();
export default function ProfessionalReviewsCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setReviewedId(initialValues.ReviewedId);
    setReviewType(initialValues.ReviewType);
    setProfessionalName(initialValues.ProfessionalName);
    setProfessionalRole(initialValues.ProfessionalRole);
    setReviewDate(initialValues.ReviewDate);
    setErrors({});
  };
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createProfessionalReviews.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "ProfessionalReviewsCreateForm")}
      {...rest}
    >
      <TextField
        label="Reviewed id"
