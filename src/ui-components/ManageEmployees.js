...
export default function OrganizationEmployeesTable() {
  ...
  const [isCurrentUserPrimaryAdmin, setIsCurrentUserPrimaryAdmin] =
    useState(false);
      ...
      try {
        ...
        if (accountType === "Professional" && userEmail) {
          ...
          if (matchingRecord) {
            ...
            setIsCurrentUserPrimaryAdmin(
              !!matchingRecord.PrimaryOrganizationAdministrator
            );
          }
    ...
    try {
      ...
      for (const row of rows) {
        try {
          ...
          let businessName = "Your Organization";

          try {
            const orgRes = await client.graphql({
              query: listOrganizationInformations, // or your org query
              variables: { filter: { OrganizationId: { eq: organizationId } } },
            });
            const orgData = orgRes.data.listOrganizationInformations.items[0];
            if (orgData?.BusinessName) businessName = orgData.BusinessName;
          } catch (err) {
            console.warn("Could not fetch business name:", err);
          }
          ...
          const generalInfo =
            generalInfoRes.data.listGeneralInformations.items[0];
          ...
          if (existingRecord?.id) {
            ...
            await fetch(
              "https://3vqzvlkgxiokrm4hkj65oefwdy0ckrdp.lambda-url.us-east-1.on.aws/",
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  to: email,
                  subject: "Create account on Wholistic Health App®!",
                  body: `Your are being invited to create an account on the Wholistic Health App by ${businessName}.`,
                  includeButton: true,
      ...
      const updatedRows = refreshed.data.listOrganizationCPs.items.map(
        (item) => ({
          email: item.OrganizationCPEmail || "",
          firstName: item.OrganizationCPFirstName || "",
          lastName: item.OrganizationCPLastName || "",
