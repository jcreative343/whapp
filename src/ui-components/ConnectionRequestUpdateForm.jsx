...
import { getConnectionRequest } from "../graphql/queries";
import { updateConnectionRequest } from "../graphql/mutations";
const client = generateClient();
export default function ConnectionRequestUpdateForm(props) {
  const {
    id: idProp,
    connectionRequest: connectionRequestModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = connectionRequestRecord
      ? { ...initialValues, ...connectionRequestRecord }
      : initialValues;
    setRequesterId(cleanValues.requesterId);
    setRequesterFirstName(cleanValues.requesterFirstName);
    setRequesterLastName(cleanValues.requesterLastName);
    setRequesterEmail(cleanValues.requesterEmail);
    setTargetEmail(cleanValues.targetEmail);
    setStatus(cleanValues.status);
    setDirection(cleanValues.direction);
    setErrors({});
  };
  const [connectionRequestRecord, setConnectionRequestRecord] = React.useState(
    connectionRequestModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getConnectionRequest.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getConnectionRequest
        : connectionRequestModelProp;
      setConnectionRequestRecord(record);
    };
    queryData();
  }, [idProp, connectionRequestModelProp]);
  React.useEffect(resetStateValues, [connectionRequestRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateConnectionRequest.replaceAll("__typename", ""),
            variables: {
              input: {
                id: connectionRequestRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ConnectionRequestUpdateForm")}
      {...rest}
    >
      <TextField
        label="Requester id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || connectionRequestModelProp)}
