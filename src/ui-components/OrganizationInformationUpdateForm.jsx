...
import { getOrganizationInformation } from "../graphql/queries";
import { updateOrganizationInformation } from "../graphql/mutations";
...
export default function OrganizationInformationUpdateForm(props) {
  const {
    id: idProp,
    organizationInformation: organizationInformationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = organizationInformationRecord
      ? { ...initialValues, ...organizationInformationRecord }
      : initialValues;
    ...
    setProgramsUnits(cleanValues.ProgramsUnits ?? []);
    setCurrentProgramsUnitsValue("");
    setServices(cleanValues.Services ?? []);
    setCurrentServicesValue("");
    setClientsServed(cleanValues.clientsServed);
    setErrors({});
  };
  const [organizationInformationRecord, setOrganizationInformationRecord] =
    React.useState(organizationInformationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getOrganizationInformation.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getOrganizationInformation
        : organizationInformationModelProp;
      setOrganizationInformationRecord(record);
    };
    queryData();
  }, [idProp, organizationInformationModelProp]);
  React.useEffect(resetStateValues, [organizationInformationRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          ProgramsUnits: ProgramsUnits ?? null,
          Services: Services ?? null,
          clientsServed: clientsServed ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateOrganizationInformation.replaceAll("__typename", ""),
            variables: {
              input: {
                id: organizationInformationRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "OrganizationInformationUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || organizationInformationModelProp)}
