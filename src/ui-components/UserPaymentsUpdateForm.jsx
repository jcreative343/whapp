...
import { getUserPayments } from "../graphql/queries";
import { updateUserPayments } from "../graphql/mutations";
const client = generateClient();
export default function UserPaymentsUpdateForm(props) {
  const {
    id: idProp,
    userPayments: userPaymentsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userPaymentsRecord
      ? { ...initialValues, ...userPaymentsRecord }
      : initialValues;
    ...
    setClientsProjected(cleanValues.clientsProjected);
    setCostPerClient(cleanValues.costPerClient);
    setCouponCode(cleanValues.couponCode);
    setCouponPercent(cleanValues.couponPercent);
    setPaidUntil(cleanValues.paidUntil);
    setPaymentFor(cleanValues.paymentFor);
    setErrors({});
  };
  const [userPaymentsRecord, setUserPaymentsRecord] = React.useState(
    userPaymentsModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getUserPayments.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserPayments
        : userPaymentsModelProp;
      setUserPaymentsRecord(record);
    };
    queryData();
  }, [idProp, userPaymentsModelProp]);
  React.useEffect(resetStateValues, [userPaymentsRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          clientsProjected: clientsProjected ?? null,
          costPerClient: costPerClient ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateUserPayments.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userPaymentsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserPaymentsUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userPaymentsModelProp)}
